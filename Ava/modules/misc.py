...
from Ava import OWNER_ID, dispatcher
from Ava.modules.disable import DisableAbleCommandHandler
from Ava.modules.helper_funcs.alternate import send_action, typing_action
...
def src(update, _):
    update.effective_message.reply_text(
        "Hey there! You can find what makes me click [here](https://github.com/doraemon890/AvaRobot).",
        parse_mode=ParseMode.MARKDOWN,
        disable_web_page_preview=True,
    )
...
def wiki(update, context):
    ...
    if not str(Shinano[1]):
        ...
        try:
            Ava = update.effective_message.reply_text(
