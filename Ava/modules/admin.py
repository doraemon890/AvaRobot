...
from Ava import Jarvis, dispatcher
from Ava import telethn as bot
...
from Ava.modules.helper_funcs.extraction import extract_user, extract_user_and_text
...
async def admins(client, message):
    try:
        adminList = []
        ownerList = []
        async for admin in Jarvis.get_chat_members(
        ...
        await Jarvis.send_message(message.chat.id, text2)
...
async def bots(client, message):
    try:
        botList = []
        async for bot in Jarvis.get_chat_members(
        ...
        await Jarvis.send_message(message.chat.id, text3)
    except FloodWait as e:
        await asyncio.sleep(e.value)
