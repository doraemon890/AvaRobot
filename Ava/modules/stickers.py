...
async def drawText(image_path, text):
    ...
    fnt = "ariel.ttf" if os.name == "nt" else "./Ava/modules/resources/asu.ttf"
...
async def _(event):
    ...
    im1 = Image.open("Ava/modules/resources/blank_background.png")
    if ik.endswith(".tgs"):
        await bot.download_media(reply, "blank_background.tgs")
        ...
        file = "o.webp"
        os.remove("k.png")
    await bot.send_file(event.chat_id, file, reply_to=event.reply_to_msg_id)
    await kontol.delete()
    os.remove(file)
    os.remove(ik)
