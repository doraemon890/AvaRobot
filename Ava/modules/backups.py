...
import Ava.modules.sql.blacklist_sql as blacklistsql

import Ava.modules.sql.locks_sql as locksql
import Ava.modules.sql.notes_sql as sql

import Ava.modules.sql.rules_sql as rulessql
from Ava import JOIN_LOGGER, LOGGER, OWNER_ID, SUPPORT_CHAT, dispatcher
from Ava.__main__ import DATA_IMPORT
from Ava.modules.connection import connected
from Ava.modules.helper_funcs.alternate import typing_action
from Ava.modules.helper_funcs.chat_status import user_admin
from Ava.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open(f"Ava-Ava{chat_id}.backup", "w") as f:
        f.write(baccinfo)
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open(f"Ava-Ava{chat_id}.backup", "rb"),
        caption=f"*sᴜᴄᴄᴇssғᴜʟʟʏ ᴇxᴘᴏʀᴛᴇᴅ ʙᴀᴄᴋᴜᴘ:*\nᴄʜᴀᴛ: `{chat.title}`\nᴄʜᴀᴛ ɪᴅ: `{chat_id}`\nᴏɴ: `{tgl}`\n\nɴᴏᴛᴇ: ᴛʜɪs `Ava-Ava-Backup` ᴡᴀs sᴘᴇᴄɪᴀʟʟʏ ᴍᴀᴅᴇ ғᴏʀ ɴᴏᴛᴇs.",
        timeout=360,
        reply_to_message_id=msg.message_id,
        parse_mode=ParseMode.MARKDOWN,
    )
    os.remove(f"Ava-Ava{chat_id}.backup")
