...
from Ava import OWNER_ID, SUPPORT_CHAT, Jarvis
from Ava.utils.errors import split_limits
    ...
    async def capture(client, message, *args, **kwargs):
        try:
            ...
            for x in error_feedback:
                await Jarvis.send_message(SUPPORT_CHAT, x)
            raise err
...
async def member_permissions(chat_id, user_id):
    perms = []
    member = await Jarvis.get_chat_member(chat_id, user_id)
...
async def current_chat_permissions(chat_id):
    perms = []
    perm = (await Jarvis.get_chat(chat_id)).permissions
